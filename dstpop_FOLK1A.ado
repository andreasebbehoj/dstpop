...
program define dstpop_FOLK1A, rclass
version 15
...
** Marital status
if !mi("`civilstand'") {
    local api_civilstand =	"&CIVILSTAND=" 	+ subinstr("U G E F", " ", "%2C", .)
}
...
** Sex
if !mi("`koen'") {
    local api_koen = 		"&K%C3%98N=" 	+ subinstr("1 2", " ", "%2C", .)
}
...
*** Output 
return local apiurl = "https://api.statbank.dk/v1/data/FOLK1A" /// URL to registry´s API
	/// Common settings
	+ "/CSV?" /// csv format
	+ "lang=en" ///
